...
SOLUTION_FILE = 'Nancy.Bootstrappers.Ninject.sln'
...
xunit :xunit => [:compile] do |xunit|
    tests = FileList["test/**/#{CONFIGURATION}/Nancy.Bootstrappers.Ninject.Tests.dll"].exclude(/obj\//)
...
zip :package => [:publish] do |zip|
    ...
    zip.output_file = "Nancy.Bootstrappers.Ninject-Latest.zip"
...
task :nuget_package => [:publish] do
    ...
    nuspecs = FileList["src/nancy.bootstrappers.Ninject/*.nuspec"]
    root = File.dirname(__FILE__)

    # Copy all project *.nuspec to nuget build folder before editing
    FileUtils.cp_r nuspecs, "#{OUTPUT}/nuget"
    nuspecs = FileList["#{OUTPUT}/nuget/nancy.bootstrappers.Ninject.nuspec"]
    ...
    nuspecs.each do |nuspec|
        update_xml nuspec do |xml|
            ...
            xml.root.elements["metadata/licenseUrl"].text = "https://github.com/NancyFx/Nancy.Bootstrappers.Ninject/blob/master/license.txt"
